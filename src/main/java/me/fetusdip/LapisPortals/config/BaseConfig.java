...
public abstract class BaseConfig {
    ...
    protected static void logError(String s) {
        Bukkit.getLogger().severe(config.getName() + ".yml: " + s);
    }

    protected static void log(String s) {
        Bukkit.getLogger().info(config.getName() + ".yml: " + s);
    }
    ...
    protected static int[] getRangedValue(String path, int expected, String def) {
        ...
        String[] value = Objects.requireNonNull(config.getString(path)).split(":", expected);

        if (value.length == 0){
            logError("No range found for, " + path);
        }

        int[] arr = new int[expected];
        try {
            for (int x = 0; x < arr.length; x++) {
                arr[x] = Integer.parseInt(value[x]);
            }
